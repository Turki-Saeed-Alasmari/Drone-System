...
double current_load; 
 ...
 class Package
 {
    private:
        static int id;
        double  weight;
    friend ostream& operator<<(ostream& out, const Package& d);  	friend class Route; 
 
 public:
        Package();
        Package(double weight);  	~Package();  	void getdata();  	double putdata();
...
int Drone::id = 0; 
    ...
    {
        id++;
        this->capacity = capacity;  	this->current_load = current_load;
    }
    Drone::~Drone()
    {
        capacity = 0.0;
    ...
    Drone Drone:: operator + (Drone obj)
    {
        Drone res;
        //res.id = id + obj.id; 
        res.capacity = capacity + obj.capacity;  	res.current_load = current_load + obj.current_load;  	return res;
    }
    ...
    int main()
    {
        ...
        do {
            do
            {
                cout << "Enter The capacity of a drone should be a positive number : ";  	 	 	cin >> capacity;
            } while (capacity <= 0);
        ...
        Drone  ob1(capacity, current_load);  cout << ob1 << endl;
        ...
        {
            Package total;
            cout << "Enter Package1  weight : ";  	 	p1.getdata();
            cout << "Enter Package2  weight : ";  	 	p2.getdata();
            total = total + p1;
            ...
            if ((total.putdata() + current_load) > capacity)
            {
                ...
                cout << "capacity :  " << capacity << endl;  	 	 	weighterror = true;
            }
        } while (weighterror);

        char Rut1[] = { 'A','B','C','D' };  	char Rut2[] = { 'X','Y','Z' };
        ...
        cout << "\np1 delivery " << endl;  	Route R2(p2, Rut2);
        for (int i = 0; i < 3; i++)
        {
